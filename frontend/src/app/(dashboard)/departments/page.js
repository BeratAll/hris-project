...
export default function DepartmentsPage() {
  ...
  const departmentError = useSelector(selectDepartmentsError);
  ...
  // Sayfa yüklendiğinde departmanları çek
  useEffect(() => {
  ...
  /**
   * Yeni Departman Ekleme Modalini Aç
   */
  ...
  /**
   * Departman Düzenleme Modalini Aç
   */
  ...
  const handleSubmit = async (e) => {
    ...
    if (selectedDepartment) {
      ...
      result = await dispatch(addDepartment(formValues.name));
  ...
  const handleDeleteConfirm = async () => {
    if (departmentToDelete) {
      const result = await dispatch(deleteDepartment(departmentToDelete.id));
    ...
    <Box sx={{ width: '100%' }}>
      ...
      >
        <Box>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
            Organizasyon yapısı içerisindeki aktif ve pasif departmanların yönetimi
          </Typography>
        </Box>
        <Button
          variant="contained"
      ...
      >
        <DataGrid
          rows={departments}
          columns={columns}
          loading={status === 'loading' && departments.length === 0}
          initialState={{
            pagination: {
      ...
      >
        <DialogTitle id="department-dialog-title" sx={{ fontWeight: 700 }}>
        ...
        <Box component="form" onSubmit={handleSubmit}>
          ...
          <DialogActions sx={{ px: 3, py: 2 }}>
            ...
            <Button type="submit" variant="contained" sx={{ fontWeight: 600 }} disabled={status === 'loading' || !formValues.name.trim()}>
              {status === 'loading' ? 'Kaydediliyor...' : selectedDepartment ? 'Güncelle' : 'Kaydet'}
            </Button>
          </DialogActions>
        </Box>
      </Dialog>
      ...
      >
        ...
        <DialogContent>
          <Typography>
            <strong>{departmentToDelete?.name}</strong> departmanını silmek istediğinize emin misiniz? Bu işlem geri alınamaz.
          </Typography>
        </DialogContent>
        <DialogActions sx={{ px: 3, py: 2 }}>
